...
vocabSize=44603
heldAndTestFile=$3
textFactorForNWTextBalance=1
zeroEdges=$4
diagHyperInit=2.5
nonDiagHyperInit=1.5
seedIndexFileName="null"
vocabFile="cancer_vocab.txt"
alpha=0.01
dateTime=$(date +"%m-%d-%y")
faster=0
#45 threads, 245 users
threads=$5
outputFile=${input_file}-numParallel_${numParallelThread}-zeroEdges_$zeroEdges-threads_$threads-$dateTime.csv
dumpFile=${outputFile}.dump

echo -e "input_file:${input_file}\t num_topics: ${num_topics}\t outer_iter ${outer_iter}\t inner_iter ${inner_iter}\t nu_iter ${nu_iter}\t stepSizeNu $stepSizeNu\t stochastic_step_tau ${stochastic_step_kappa}\t sampling_threshold ${sampling_threshold}\t stochastic_step_kappa ${stochastic_step_tau}\t numParallelThread $numParallelThread\t outputFile $outputFile\t vocabSize $vocabSize\t heldAndTestFile $heldAndTestFile\t textFactorForNWTextBalance $textFactorForNWTextBalance\t zeroEdges $zeroEdges\t diagHyperInit $diagHyperInit\t nonDiagHyperInit $nonDiagHyperInit\t seedIndexFileName $seedIndexFileName\t vocabFile $vocabFile\t alpha $alpha\t faster $faster\t threads $threads"

./ldaMmsb.out ${input_file} ${num_topics} ${outer_iter} ${inner_iter} ${nu_iter} $stepSizeNu ${stochastic_step_kappa} ${sampling_threshold} ${stochastic_step_tau} $numParallelThread $outputFile $vocabSize $heldAndTestFile $textFactorForNWTextBalance $zeroEdges $diagHyperInit $nonDiagHyperInit $seedIndexFileName $vocabFile $alpha $faster $threads
