...
void getGraclusFormat(std::unordered_map< std::pair<int,int>, std::unordered_map<int,int>*, class_hash<pair<int,int>>>* userAdjlist, std::unordered_map<int,int>* userList, char* filename, 
	...
	std::unordered_map<int, std::unordered_set<int>*>* globalAdjList = new std::unordered_map<int, std::unordered_set<int>*>();
//	matrix<int>* adjacencyMatrix = new matrix<int>(userList->size(), userList->size());
//	cout<<"Created adjacencyMatrix\n";
...
//	}
	for(std::unordered_map< std::pair<int,int>, std::unordered_map<int,int>*, class_hash<pair<int,int>>>::iterator it1=userAdjlist->begin(); it1!=userAdjlist->end(); ++it1){
		for(std::unordered_map<int,int>::iterator it2 = it1->second->begin(); it2!=it1->second->end(); ++it2){
//			int count = (weighted)? it2->second : 1;
//			(*adjacencyMatrix)(userList->at(it1->first.first), userList->at(it2->first)) = count;
			int user_i = userList->at(it1->first.first)+1;
			int user_j = userList->at(it2->first)+1;
			if(globalAdjList->count(user_i)>0)
				globalAdjList->at(user_i)->insert(user_j);
			else{
				std::unordered_set<int>* user_set = new std::unordered_set<int>();
				user_set->insert(user_j);
				globalAdjList->insert({user_i,user_set});
			}
			if(globalAdjList->count(user_j)>0)
				globalAdjList->at(user_j)->insert(user_i);
			else{
				std::unordered_set<int>* user_set = new std::unordered_set<int>();
				user_set->insert(user_i);
				globalAdjList->insert({user_j,user_set});
			}
//			outfile << it1->first.first <<" "<<it2->first<<" "<<count<<endl;
//			outfile<<endl;
		}
	}
	...
	for(int i=1; i<=userList->size(); ++i){
		edgeCounts+=globalAdjList->at(i)->size();
//		for(int j=0; j<userList->size(); ++j){
//			if((*adjacencyMatrix)(i, j)>0)
//				edgeCounts++;
//		}
	}
	cout<<"After second For\n";
    outfile<<userList->size()<<" "<<edgeCounts/2<<endl;
	for(int i=1; i<=userList->size(); ++i){
		outfile<<userIndexMap->at(i-1);
//		for(int j=0; j<userList->size(); ++j){
//			if((*adjacencyMatrix)(i, j)>0)

		for(std::unordered_set<int>::iterator it=globalAdjList->at(i)->begin(); it!=globalAdjList->at(i)->end(); ++it){
			outfile<<" "<<(*it);//j;
		}
		outfile<<endl;
	}
...
void getAdjacencyMatrix(std::unordered_map< std::pair<int,int>, std::unordered_map<int,int>*, class_hash<pair<int,int>>>* userAdjlist, std::unordered_map<int,int>* userList, char* filename, 
		std::unordered_map<int,int>* userIndexMap, bool weighted){
//	matrix<double>* adjacencyMatrix = new matrix<double>(userList->size(), userList->size());
	ofstream outfile(filename);
	for(std::unordered_map< std::pair<int,int>, std::unordered_map<int,int>*, class_hash<pair<int,int>>>::iterator it1=userAdjlist->begin(); it1!=userAdjlist->end(); ++it1){
		for(std::unordered_map<int,int>::iterator it2 = it1->second->begin(); it2!=it1->second->end(); ++it2){
			int count = (weighted)? it2->second : 1;
//			(*adjacencyMatrix)(userList->at(it1->first.first), userList->at(it2->first)) = it2->second;
			outfile << it1->first.first <<" "<<it2->first<<" "<<count<<endl;
//			outfile<<endl;
		}
	}
...
//    getAdjacencyMatrix(userAdjlist, userList, argv[2], userIndexMap, (atoi(argv[3])==1));
    getGraclusFormat(userAdjlist, userList, argv[2], userIndexMap, (atoi(argv[3])==1));
