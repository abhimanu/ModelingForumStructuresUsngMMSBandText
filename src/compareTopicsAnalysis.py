...
def getTopicSingleGibbs(threadStarterResultsFile1):
	...
	for topic in xrange(1,numTopics+1):
		...
		topicWord1[int(topic)] = vocabArray
...
#	for factorLoop in xrange(1,30):		# one less than the total
	topicWord = getTopicSingleGibbs(ldaFileString)
	for topic1 in xrange(1,numTopics+1):		# one less than the total
		vocab1=topicWord[topic1]
		for topic2 in xrange(1,numTopics+1):
			vocab2=topicWord[topic2]
			...
			resultsCompare.write(str(topic1)+" "+str(topic2)+ " "+ str(similar)+"\n")
			print str(topic1)+" "+str(topic2)+ " "+ str(similar) 
