...
//	matrix<double>* held_phi_lgammaPhi;
	...
	std::vector<double>* lda_prediction_error_thread_list;
...
//	matrix<int>* inputMat;
	...
	double lda_heldoutPredictionError = 0.0;
...
//	void updateB(int p, int q, matrix<double>* oldPhi_pq);
	...
	double getParallelSimpleLDAprediction(int threadID);
	std::vector<double>* getSimpleLDATopicVector(std::pair<int,int> user_thread, std::vector<double>* constDigamma, std::vector<double>* user_topic);
...
//	void updateAlpha(bool flagLL);
	...
	void setSimpleLDAPredcitDiag(double simpleLDAPredcitDiag);
};
...
void MMSBpoisson::setSimpleLDAPredcitDiag(double simpleLDAPredcitDiag){
	this->simpleLDAPredcitDiag = simpleLDAPredcitDiag;
}
...
double MMSBpoisson::getParallelSimpleLDAprediction(int threadID){
	double predictionError=0.0;
	lda_prediction_error_thread_list->at(threadID) =0;//+= (abs(Y_pq-Y_predicted));
	...
	for(std::unordered_map< std::pair<int,int>, std::unordered_map<int,int>*, class_hash<pair<int,int>>>::iterator it1=heldUserAdjlist_thread_list->at(threadID)->begin(); it1!=heldUserAdjlist_thread_list->at(threadID)->end(); ++it1){
		for(std::unordered_map<int,int>::iterator it2 = it1->second->begin(); it2!=it1->second->end(); ++it2){
			...
			int Y_pq = it2->second + inputCountOffset;
			int q = userList->at(it2->first);
			std::pair<int,int> user_thread_p = it1->first;
			std::pair<int,int> user_thread_q = std::make_pair(it2->first,it1->first.second);
			if(userThreadPost->count(user_thread_q)<=0){
			//	cout<<"user doesnt have any posts in thread "<<it2->first<<"; "<<it1->first.second<<endl;
				lda_prediction_error_thread_list->at(threadID) += (abs(Y_pq-0));
				continue;
			}
			std::vector<double>* topic_vector_p = new std::vector<double>(K);
			std::vector<double>* topic_vector_q = new std::vector<double>(K);
			topic_vector_p = getSimpleLDATopicVector(user_thread_p, constDigamma, topic_vector_p);
			topic_vector_q = getSimpleLDATopicVector(user_thread_q, constDigamma, topic_vector_q);
			double Y_predicted = 0;//getPredictionForEdge(meanBlockMat,p,q);
			for(int k=0; k<K; k++){
				Y_predicted += topic_vector_p->at(k)*topic_vector_q->at(k)*simpleLDAPredcitDiag;
			}
			lda_prediction_error_thread_list->at(threadID) += (abs(Y_pq-Y_predicted));
		}
	}
}

std::vector<double>* MMSBpoisson::getSimpleLDATopicVector(std::pair<int,int> user_thread, std::vector<double>* constDigamma, std::vector<double>* user_topic){
	...
	for(std::vector<int>::iterator it = userThreadPost->at(user_thread)->begin(); it!=userThreadPost->at(user_thread)->end(); ++it){
		...
		for(int k=0; k<K; k++){
			...
			chi_tpi->at(k) = exp(getDigamaValue((*tau)(k,wordId)) - getDigamaValue(constDigamma->at(k)) );
...
//	std::vector<std::unordered_map<int,int>*>* 
	...
	lda_prediction_error_thread_list = new std::vector<double> (numParallelThreads);
...
//			}
			...
			cout<<"LDA_prediction error: "<<lda_heldoutPredictionError/numHeldoutEdges<<"\n"<<flush;
...
//	cout<<"Start of method multiThreadGlobalMatsFromLocal"<<endl;
	...
	for(int thr=0; thr<numParallelThreads; thr++){
		...
		lda_heldoutPredictionError += lda_prediction_error_thread_list->at(thr);
	}
...
//		cout<<"\t In thread "<<threadID<<";\t";
		...
		getParallelSimpleLDAprediction(threadID);
		clock_t end = clock();
