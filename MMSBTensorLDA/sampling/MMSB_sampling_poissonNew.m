function [alpha, B, pi, eta] = MMSB_sampling_poissonNew(Y,K,sampleIters, outer, L, filename)
% prior for B
...
for inner_iter=1:sampleIters
    inner_iter
    [Nkk,Nuk,Zuu,Sum_kk] = sampler(Nkk,Nuk,Zuu,eta,alpha,num_users,Y,K,Sum_kk,kappa);
%     Zuu
...
%     x1
%     y1
    ll(inner_iter)=calculate_joint_log_likelihood(Nkk,Nuk,K,num_users,alpha,eta,kappa,Sum_kk,Zuu,Y);
%     Nkk
%     Nuk
    ll(inner_iter)
%     sum(sum(Sum_kk))
...
% save(strcat(filename,'_LL.mat'),'ll');
...
function [ll]=calculate_joint_log_likelihood(Nkk,Nuk,K,num_users,alpha,eta,kappa,Sum_kk,Zuu,Y)
ll=0;
% Nkk
...
%     [factorial_x,factorial_y] = find(Zuu)
    for h=1:K        
        [sumLnGammaY_pq, sum_gh] = getSumLnGammaY_pq(Zuu, g, h, Y);
%         Sum_kk(g,h)
        if sum_gh~=Sum_kk(g,h)
            'screwed badly'
            return;
        end
        ll=ll +gammaln(Sum_kk(g,h)+kappa) -(Sum_kk(g,h)+kappa)*(Nkk(g,h)+1/eta)-kappa*log(eta)-gammaln(kappa); %- sumSigma gammaln(Y_pq+1)    
        ll = ll-sumLnGammaY_pq;
    end
end
...
function [Nkk,Nuk,Zuu,Sum_kk] = sampler(Nkk,Nuk,Zuu,eta,alpha,num_users,Y,K,Sum_kk,kappa)
for user_p=1:num_users
    for user_q=1:num_users
        ...
        multi_probs=calculate_Mult_Probs(Nkk,Nuk(user_p,:),Nuk(user_q,:),K,y_pq,eta,alpha,Sum_kk,kappa,p_k,q_k);
...
function [multi_probs]=calculate_Mult_Probs(Nkk,Npk,Nqk,K,y_pq,eta,alpha,Sum_kk,kappa,curr_p,curr_q)
...
%         end
        ...
        p=1/((1/eta)+n_kk+1);
        p=1-p;
        sum_local = sum_local + kappa;
%         sum_local
%         p
%         y_pq
        block_part = log(nbinpdf(y_pq,sum_local,p));        % nbinpdf returns zero if y_pq is not and integer
%         block_part;
